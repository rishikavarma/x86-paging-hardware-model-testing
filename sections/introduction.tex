...
All system software must directly interact with hardware to establish the desired isolation properties, e.g., by setting up a virtual machine or a process's address space. 
...
Consequently, the verification result depends directly on the accuracy of the hardware specification against which it is verified.
...
To ease the analysis of a large number of long, complex traces, we propose automating validation by framing it as a software synthesis problem, thereby preventing manual intervention.
For this, instead of synthesizing instructions, we synthesize the invisible, internal microarchitectural steps and use the observable trace as the verification condition.
...
To reduce the number of choices in test case generation and validation, we employ techniques that leverage symmetry and abstraction exploitable in the hardware model.
Intuitively, we can replace a trace with a simpler one if it still exposes the same hardware behavior.
For example, consider an execution trace of memory accesses in an eight-core system; we might be able to find an equivalent trace on a two-core system by combining execution steps or breaking the trace into smaller test traces.
...
Finally, we present our choice-reduction techniques applicable to our model, which reduce the number of cores, virtual pages, physical pages, and page-table entries. 
...
\item A reduction strategy enabling tractable validation of the x86 MMU model adaptable to other models (\S\ref{sec:reduction})
\item A prototype implementation evaluated on x86 paging behavior (\S\ref{sec:eval})
\end{itemize}
