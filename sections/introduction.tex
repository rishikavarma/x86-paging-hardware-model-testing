...
% 
System software is a high-value target for formal verification due to its critical role in enforcing isolation between processes, containers, and virtual machines.
There has been a steady stream of verified kernels~\cite{Klein:2009:seL4, Gu:2016:CertiKOS, Chen:2025:Atmosphere}, hypervisors~\cite{Tao:2021:seKVMArm}, and security monitors~\cite{Zhou:2024:Verismo}.
...
We then use this specification to reason about the effects of a write to a register or a page table entry on the overall system, e.g., whether the process now has access to a given page of memory.
...
Second, there is a reality gap between the formal model and the actual hardware~\cite{Gleirscher:2019:RealityGap}, i.e., the formal model does not contain bugs, but it is either too permissive or too restrictive. The verification effort becomes much harder than necessary, or the model abstracts away relevant details.
...
the TSO memory model~\cite{owens2009x86tso}, or the Arm weak memory models~\cite{alglave2011litmus,alglave2014herding}.
However, none of them explicitly include the invisible processor-internal steps of the virtual memory system.
...
2) executing these test cases on real hardware, and
3) validating the model against a trace of observed hardware behavior.
