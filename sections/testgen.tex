...
We use the constraints from the model reduction to select arguments for reading and writing memory addresses and page table entries.\hl{For example, since we limit the number of virtual addresses to three, we pick one of these values when the test calls for a memory read. We also ensure to pick the values such that there is no repetition in the same type of test patterns but with different addresses that dont expose new scenarios.}
