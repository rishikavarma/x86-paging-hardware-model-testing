...
\begin{itemize}
  ...
  A useful abstraction is to think of a walk as alternating \emph{remote} paging-structure writes $R$ (from other cores) with level transitions $L_i$, e.g.\ $\textsc{Walk} := [R]\,L_1\,[R]\,L_2\,\cdots$ (some $L_i$ may be skipped when served from a cache), where each $R$ is one or more writes to page-table pages that are actually on the walk path for that translation.
  ...
  That aligns with the spirit of the prototype encoding in \texttt{trace\_test.rkt}---a fixed grammar over internal steps---but pushes the grammar toward fewer, semantically meaningful families of steps instead of maximally fine-grained ones.
\end{itemize}
