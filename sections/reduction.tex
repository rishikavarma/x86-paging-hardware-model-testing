...
To make test generation and validation tractable, we reduce the hardware model by limiting the range of values for various aspects without loss of generality.
Next, we present each aspect of the model along with informal arguments justifying each reduction.
While these are not rigorous proofs, they are a principled application of a small-scope argument where behaviors in a large state space are often expected to have small witnesses.

\noindent\textbf{Two cores.}
On a multi-core system, the cores share access to memory and even use the same page table. At first, this means we cannot reduce the number of cores. 
...
Using the observation above, we do not need to model all cores with the full fidelity. Instead, we can reduce the second core to a sequence of memory write backs that can happen at any time in the trace.
...
Page walk caches may skip one level and directly jump to the leaf level containing the address of the physical page. Superpages change the entry type of an upper-level page table that now contains the physical address of the page rather than pointing to the next level. 
We can again leverage symmetry and reduce the relevant page table levels to two. 

\noindent\textbf{Three Virtual Pages (or PTEs).}
Each virtual page maps to a specific leaf-level page table entry. Each virtual page has a distinct path the page table walker takes during address translation. The important factor here is the shared prefix. While we can consider paths that diverge at the root largely independent, paths that share a prefix are of interest as they will populate the page walk caches. Hence, the common prefix of the path might be skipped due to those caches.
We select three virtual pages, two located on the same PT page, the other on a different PT page covering earlier pre-fix divergence.


\noindent\textbf{Four Virtual Addresses.}
The x86 MMU does not translate on the byte-level, instead it operates at page granularity. Given, we reduced the number of virtual pages to three, we should select one representative address of each page. In addition, we add one additional address on one of the pages, exposing the a TLB-cached virtua-to-physical mapping.

\noindent\textbf{Four Physical Frames.}
These are the corresponding physical memory frames where the virtual pages map to. Here we must be able to remap an existing mapping of the virtual pages without creating an alias. Hence we need to use one more physical frame than we use virtual frames.

% \noindent\textbf{Four Physical Words.}
% These are the location in memory where the data is actually stored. 
