...
Fortunately, the internal transitions are core-local and not directly observable by other cores. Instead, other cores observe a sequence of memory accesses that originate either from the program's load/store instructions or from a core's internal steps. This provides an opportunity: from a specific core's point of view, it does not matter which other core emitted the memory access. Thus, we can merge the memory accesses of other cores into a single memory access sequence where the nondeterministic ordering results in a permutation of that order. Therefore, we can reduce the number of cores to two.

\noindent\textbf{Core Fidelity.} 
Using the observation above, we do not need to model all cores with full fidelity. Instead, we can reduce the second core to a sequence of memory write-backs that can happen at any time in the trace.
...
2) expose the effect of superpages in the mapping. 
Page walk caches may skip one level and jump directly to the leaf level containing the physical page's address. Superpages change the entry type of an upper-level page table entry so that it now contains the page's physical address rather than pointing to the next level. 
