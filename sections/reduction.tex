...
Next, we present each aspect of the model along with tentative informal arguments justifying each reduction.
...
The critical aspect here is that we can model the non-atomicity of a page table walk. Specifically,  
1) we can model the effect of the page walk caches that skip the upper levels of the page table on the page walk, and 
2) exposing the effect of super pages in the mapping. 
Page walk caches may skip one level and jump directly to the leaf level containing the physical page's address. Superpages change the entry type of an upper-level page table so that it now contains the page's physical address rather than pointing to the next level. 
We can again leverage symmetry and reduce the relevant page table levels to two. 
