...
%
Unfortunately, this model has several problems.
...
As a result, the MMU may map virtual addresses to the wrong physical address or apply the stale (wrong) permissions during access control, breaking isolation and violating system security.
...
Consequently, the hardware may exhibit behavior for which there exists no corresponding operational step in the specification, and verification against such an overapproximated specification yields an implementation that does not correctly handle behavior present on real hardware.
...
While this covers all behavior of the underlying hardware, it also forces reasoning within a less permissive environment specification, which may make the proof effort more substantial.
