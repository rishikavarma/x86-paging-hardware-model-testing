...
We could model this as two flat, mathematical maps: one representing the TLB and one storing a flattened representation of the page table's virtual-to-physical address translation. Then, we express operations to update a page table entry, fill and invalidate the TLB, and perform a memory access that uses the translation information in the TLB. 
%
Unfortunately, this model has several problems \hl{in accurately depicting certain important behaviors}.
...
Store buffers and data caches are omitted. Updates to the page table are modeled as atomic and become globally visible to all cores simultaneously.
...
\hl{As a result, the MMU may not see updates to the page table immediately and maps virtual addresses to the wrong physical address or applies the stale (wrong) permissions during access control, breaking isolation and violating system security.}
% Check above once
...
Consequently, the hardware may exhibit behavior for which there exists no corresponding operational step in the specification, and verification against such an overapproximated specification yields an implementation that does not correctly handle behavior present on real hardware. \hl{This can result in unsafe system behaviors leading to security violations.}
...
While this covers all behavior of the underlying hardware, it also forces reasoning within a less permissive environment specification, which may make the proof effort more substantial \hl{or impractical}.


\subsection{The Environment Specification must be Tight}

In summary, the environment specification must be able to express the hardware's possible behavior, nothing more.
A too strict model leads to meaningless verification results because the verified program runs in an environment it cannot handle, and thus \hl{subsequent behavior may diverge from expected, safe patterns}~\cite{goldweber2024ironspec,reid2016trustworthy}. 
