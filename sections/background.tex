...
This address mapping is stored in the page table, a memory-resident, multi-level radix tree.

% translations can be cached
\noindent\textbf{Translation Caching.}
The x86 MMU has two kinds of caches that speed up address translation: 
1) the translation lookaside buffer (TLB) stores the result of successful page table walks, and 
2) the page-walk cache (PWC) stores partial page table walks. 
...
This lets the processor retire store instructions without waiting for the memory subsystem.
...
A processor effectively executes a sequence of events, which we can classify into externally observable and invisible internal events.
